 ...
 * @LastEditTime: 2021-09-15 16:56:08:088:354:295
 */
...
(function () {
  ...
  hm.src = 'https://hm.baidu.com/hm.js?30606b57e40fddacb2c26d2b789efbcb';
...
(function (global) {
  function NEWDSP() {
    /**
     * 请求参数示例
     * 在 formattingConfig 方法中根据传递的参数重新修改参数
     *
     * required: 必填字段
    ...
    }; // 用户传递的初始请求参数数组，每一次请求都会记录

    this.userConfigArr = []; // 使用过的参数数组，每一次修改后都会 push 一次，包括初始的参数

    this.configArr = []; // 浏览器信息

    this.browserInfo = this.IEBrowserVersion(); // 上报对象

    this.dspReport = {}; // 记录所有请求到的 dsp 数据，包括 新萌 dsp
  ...
  NEWDSP.prototype = {
    ...
    GET_DSP_DATA: function GET_DSP_DATA(userConfig, callback) {
      ...
      var result = {
        ...
        // DSP 广告数据
        ending: false,
        // 请求结果: true / false 是否有数据
        info: '' // 提示信息

      };

      if (_this.paramCheck(userConfig)) {
        typeof console !== 'undefined' ? console.log('参数校验失败') : '';
      ...
      var transitiveConfig = {
        uid: userConfig.site.uid,
        // string userID
        qid: userConfig.site.qid,
        // OSType: userConfig.requestParams.param.os, // string 系统
        // page: userConfig.baseInfo.page, // 页面类型（首页index，详情页detail，分页topic，频道页channel）
        Siteid: userConfig.site.siteid || 1015,
        // string/number 站点id，平台会为每个站点分配唯一的id
        softname:
        /* userConfig.site.sitename */
        'waibu',
        // string softname
        newstype: userConfig.page.newstype,
        // string 首页传频道类型；详情页传新闻分类
        terminal: userConfig.device.terminal || 'PC',
        // string 终端：PC、WAP、APP
        deviceType: userConfig.device.deviceType || 'PC',
        // string 设备类型（默认为 PC）：PC - 个人计算机、PHONE - 手机设备、TABLET - 平板设备
        apiVersion: userConfig.site.version,
        // string 默认值1.0.1
        country: userConfig.position ? userConfig.position.country : '',
        // string 国家
        province: userConfig.position ? userConfig.position.provice : '',
        // string 省份
        city: userConfig.position ? userConfig.position.city : '',
        // string 城市
        isWaive: (userConfig.other || {
          isWaive: false
        }).isWaive || false,
        // 是否放弃请求 newdsp，直接执行后续逻辑
        isTest: (userConfig.other || {
          istest: false
        }).istest || false,
        // 判断环境 测试服 / 开发环境 || 线上
        timeout: 2000,
        // 选填 number 请求超时时间，毫秒，默认为 2000
        pageType: userConfig.page.pgtype,
        // string 页面类型（譬如：ny）
        // string/number 当前信息流屏数，如果是多屏信息流则必传；不传默认为 1
        pageNum: typeof userConfig.page.pgnum !== 'undefined' && userConfig.page.pgnum !== '' ? Number(userConfig.page.pgnum) : 1,
        imp: [],
        // Array 广告位信息 ID，根据 userConfig.baseInfo.dom 提供的位置在 _this.newdspSite 获取对应站点的 newdsp ID
        report: userConfig.report // 上报信息

      }; // 广告位信息 ID，获取对应站点的 newdsp ID

      userConfig.imparray.forEach(function (element) {
        transitiveConfig.imp.push({
          slotId: element,
          pageNum: userConfig.page.pgnum
        });
      });

      _this.formattingConfig(transitiveConfig);

      var requestConfig = _this.config; // 参数检查

      if (transitiveConfig.isWaive) {
        result.time = currentTimestamp;
        result.info = '用户放弃请求';
        callback(userConfig, result);
        return false;
      }

      requestConfig.imp = transitiveConfig.imp;
      requestConfig.requestId = String(currentTimestamp) + String(Math.random()).slice(2, 6); // 其他参数填补，可能没有

      if (transitiveConfig.user) {
        for (var key in transitiveConfig.user) {
          requestConfig.user[key] = transitiveConfig.user[key];
        }
      }

      if (transitiveConfig.device) {
        for (var _key in transitiveConfig.device) {
          requestConfig.device[_key] = transitiveConfig.device[_key];
        }
      }

      if (transitiveConfig.geo) {
        for (var _key2 in transitiveConfig.geo) {
          requestConfig.device.geo[_key2] = transitiveConfig.geo[_key2];
        }
      }
      ...
      if (transitiveConfig.app) {
        ...
        for (var _key3 in transitiveConfig.app) {
          requestConfig.app[_key3] = transitiveConfig.app[_key3];
        }
      }

      _this.recordConfig(); // 记录 config 修改后的参数
      // 请求 DSP 数据


      var url = transitiveConfig.isTest ? '//test.dpqct.com:2248/phegda/advrest' : '//open.xwdsp.com/phegda/advrest';
      ...
      _hmt.push(['_trackEvent', 'v9', 'request ', transitiveConfig.qid]);
      ...
      function success(response) {
        // 请求之后 百度统计事件
        _hmt.push(['_trackEvent', 'v9', 'response', transitiveConfig.qid]);
    ...
    // 参数校验
    paramCheck: function paramCheck(userConfig) {
      var _this = this;

      var paramType = _this.getType(userConfig);

      var result = false;
      var resultInfoArr = []; // 参考参数

      var referParam = {
        // 站点信息
        site: {
          uid: '',
          // required | String | 17 位用户ID (13 位时间戳 + 4 位随机数)
          qid: '' // required | String / Number | 渠道
          // siteid: '', // required | String / Number | 站点 ID，dsp 广告提供
          // sitename: '' // required | String | 站点名称，dsp 广告提供

        },
        // 页面信息
        page: {
          pgtype: '',
          // required | String | 页面类型 sy ny fny
          newstype: '' // required | String | 页面类型

        },
        // 其他补充信息
        // other: {
        //   istest: true // required | Boolean | 是否是开发或者测试环境，影响数据请求的接口地址
        // },
        // 广告信息
        imparray: [],
        // required | Array | 本次需要请求的广告 ID，dsp 广告提供
        // 广告上报信息，与广告信息中 ID 对应
        report: {}
      }; // 校验参数类型

      if (paramType !== 'Object') {
        resultInfoArr.push("\u53C2\u6570\u7C7B\u578B\u9519\u8BEF\uFF0C\u671F\u671B\u7C7B\u578B:Object \u53C2\u6570\u7C7B\u578B:".concat(paramType));
        result = true;
      } else {
        for (var key in referParam) {
          // 校验第一层参数
          if (typeof userConfig[key] === 'undefined') {
            // 缺少参数信息
            resultInfoArr.push("\u7F3A\u5C11\u53C2\u6570:".concat(key));
            result = true;
          } else {
            // 校验第二层参数
            if (_this.getType(userConfig[key]) === 'Object') {
              for (var key02 in referParam[key]) {
                if (typeof userConfig[key][key02] === 'undefined') {
                  // 缺少参数信息
                  resultInfoArr.push("\u7F3A\u5C11\u53C2\u6570:".concat(key, ".").concat(key02));
                  result = true;
                }
              }
            }
      ...
      if (result) {
        resultInfoArr.forEach(function (element) {
          typeof console !== 'undefined' ? console.log(element) : '';
        });
      }
    ...
    formattingConfig: function formattingConfig(acceptConfig) {
      ...
      var config = {
        ...
        // required string 终端：PC,WAP,APP
        apiVersion: acceptConfig.apiVersion || '1.0.1',
        // recommended string 默认值1.0.1
        imp: [{
          impId: undefined,
          // required string 曝光id，格式：毫秒时间戳+6位随机数（19）
      ...
      var keywords = $('[name="keywords"]').attr('content') || $('[name="Keywords"]').attr('content') || $('[name="KEYWORDS"]').attr('content') || ''; // 去除换行符

      var tempArr = keywords.replace(/\s/gi, '').split(',');
      var keywordsArr = [];
      tempArr.forEach(function (element) {
        var tempElement = $.trim(String(element));

        if (tempElement.length > 0) {
          keywordsArr.push(tempElement);
        }
      });

      if (keywordsArr.length > 0) {
        config.site.keywords = keywordsArr.join(',');
      }

      _this.delete_Object(config);

      _this.config = config; // 重新修改参数
      // dsp 上报对象

      var tempReport = acceptConfig.report || {};
    ...
    formattingDSPData: function formattingDSPData(paramObject) {
      ...
      for (var index = 0; index < adArr.length; index++) {
        var result_key = adArr[index].slotId;
        ...
        var adv_id = adArr[index].adPlanId; // type 处理
        ...
        var showbackurls = adArr[index].displayReport;
        var inviewbackurls = adArr[index].inviewReport;
        var clickbackurls = adArr[index].clickReport; // 视频
    ...
    // 记录 config 修改后的参数
    recordConfig: function recordConfig() {
      var _this = this;

      _this.configArr.push(JSON.parse(JSON.stringify(_this.config)));
    },
    // 记录所有请求到的 dsp 数据，包括 新萌 dsp
    recordDspData: function recordDspData(newData) {
      var _this = this;

      for (var key in newData.data) {
        _this.dspData["page".concat(newData.pageNum, "_").concat(key)] = newData.data[key];
      }

      _this.dspReportFn(newData);
    },
    // 上报方法
    dspReportFn: function dspReportFn(paramObj) {
      ...
      function eventFN(position) {
        ...
        if (isRepeat) {
          for (var _key4 in _this.dspReport) {
            if (new RegExp(positionName).test(_key4)) {
              eventArr.push(_this.dspReport[_key4].eventName);
            }
          }
        } else if (eventName) {
      ...
      function showFN(position) {
        ...
        _this.reportObj.show(_this.dspData[position].data);
      ...
      function viewFN(POS) {
        var positionObj = _this.dspReport[POS] || {};
        var $scrollBox = positionObj.$scrollBox || $(window); // 滚动容器盒子
        ...
        function viewEvent() {
          ...
          if (dom && isVisible && $dom_H >= 10 && rect.top - $scrollBox_T + $dom_H <= $scrollBox_H) {
            _this.reportObj.view(_this.dspData[POS].data);
      ...
      function clickFN(position) {
        ...
        $('body').on("click.click".concat(eventName), container, function () {
          _this.reportObj.click(_this.dspData[position].data);
        });
      }
    },
    reportObj: {
      show: function show(adData) {
        adData.showbackurls.forEach(function (element) {
          new Image().src = element.replace(/http:|https:/gi, '');
        });
      },
      view: function view(adData) {
        adData.inviewbackurls.forEach(function (element) {
          new Image().src = element.replace(/http:|https:/gi, '');
        });
      },
      click: function click(adData) {
        adData.clickbackurls.forEach(function (element) {
          new Image().src = element.replace(/http:|https:/gi, '');
        });
      }
    },
  ...
  global.NEWDSP_20200715 = new NEWDSP();
})(window);