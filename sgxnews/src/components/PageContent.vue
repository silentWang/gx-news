    ...
    methods:{
        reloadHome(evt){
            if(process.env.NODE_ENV != "development"){
                ...
                window.location.href = "//news.kukumai.cn/" + mode;
            }
        },
        listScroll(){
        ...
        gotoCategry(idx){
            let mode = dataCenter.getJumpToPath();
            window.open(`//news.kukumai.cn/${mode}?cateid=${idx}wow`, '_blank');
        },
        gotoNews(item){
            let mode = dataCenter.getJumpToPath();
            let turl = `//news.kukumai.cn/content/${mode}?id=${item.id}&cateid=${item.cateId}`;
        ...
        gotoPage(index){
            ...
            if(index >= len - 1){
                window.open(`//news.kukumai.cn/${mode}`, '_self');
            }
            else{
